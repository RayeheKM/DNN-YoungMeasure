...
def visualize_model_updated_minibatch(model, loss_history, term1_history, term2_history, term3_history, termc_history, x, y, z, w):
    ...
    plt.savefig('2D3_loss.png')  # Save the figure
    plt.close()  # Close the figure

    savemat('2D3_loss_data.mat', {
    ...
    for i, (x_fixed, y_fixed) in enumerate(fixed_points):
        ...
        plt.savefig(f'2D3_3d_F_x{x_fixed}_y{y_fixed}.png')  # Save each subplot
        ...
        # plt.tight_layout()
        fig.subplots_adjust(left=0.00, right=0.7, bottom=0.2, top=1)
        plt.savefig(f'2D3_3d_dFdz_x{x_fixed}_y{y_fixed}.png')  # Save each subplot
        plt.close(fig)  # Close the figure
        ...
        # plt.tight_layout()
        fig.subplots_adjust(left=0.00, right=0.7, bottom=0.2, top=1)
        plt.savefig(f'2D3_3d_dFdw_x{x_fixed}_y{y_fixed}.png')  # Save each subplot
        plt.close(fig)  # Close the figure

        # Save F surface data
        savemat(f'2D3_3d_F_x{x_fixed}_y{y_fixed}_data.mat', {
        ...
        # Save dF/dz
        savemat(f'2D3_3d_dFdz_x{x_fixed}_y{y_fixed}_data.mat', {
            'Z': Z.detach().cpu().numpy(),
            'W': W.detach().cpu().numpy(),
        ...
        # Save dF/dw
        savemat(f'2D3_3d_dFdw_x{x_fixed}_y{y_fixed}_data.mat', {
            'Z': Z.detach().cpu().numpy(),
            'W': W.detach().cpu().numpy(),
    ...
    for col, (x_value, y_value) in enumerate(fixed_points):
        ...
        plt.ylabel('Density')
        plt.tight_layout()
        plt.savefig(f'2D3_histogram_dFdz_x{x_value}_y{y_value}.png')
        plt.close(fig)
        ...
        plt.ylabel('Density')
        plt.tight_layout()
        plt.savefig(f'2D3_histogram_dFdw_x{x_value}_y{y_value}.png')
        plt.close(fig)

        savemat(f'2D3_histogram_dFdz_x{x_value}_y{y_value}_data.mat', {
            'dFdz': dF_dz.detach().cpu().numpy()
        })

        savemat(f'2D3_histogram_dFdw_x{x_value}_y{y_value}_data.mat', {
            'dFdw': dF_dw.detach().cpu().numpy()
        })
    ...
    plt.savefig('2D3_U.png')  # Save the figure
    plt.close()

    savemat('2D3_U_data.mat', {
        'X': X,
        'Y': Y,
        'U': U
    })
    ...
    # plt.savefig('2D3_U2.png')  # Save the figure
    # plt.close()
