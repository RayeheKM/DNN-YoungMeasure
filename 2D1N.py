...
def loss_function_minibatch(x1, y1, z1, w1, x2, y2, z2, w2, F):
    ...
    termCCC = (termC-termCC)**2
    termC2 = d2F_dx2_dw2 * torch.exp(-((z2**2 + w2**2)) / 2)
    termCC2 = d2F_dy2_dz2 * torch.exp(-((z2**2 + w2**2)) / 2)
    termCCC2 = (termC2-termCC2)**2
    termCComplete_vec = torch.sum((torch.sum(termCCC, dim=(0, 2, 3)) / (R1 * T1 * N1)) + (torch.sum(termCCC2, dim=(0, 2, 3)) / (R2 * T2 * N2)))
    ...
    return loss, term1Complete1_vec.item(), term2Complete1_vec.item(), term3Complete1_vec.item(), termCComplete_vec_np
