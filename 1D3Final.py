...
def visualize_model_updated(model, loss_history, term1_history, term2_history, term3_history, x, z):
    ...
    plt.savefig('1D3_loss.png')  # Save the figure
    plt.close()  # Close the figure
    
    savemat('1D3_loss_data.mat', {
    ...
    fig_3d_surface.subplots_adjust(left=0.00, right=0.7, bottom=0.2, top=1)
    plt.savefig(f'1D3_3d_F.png')  # Save each subplot
    plt.close()

    savemat('1D3_3d_F_data.mat', {
        'x': x_np,
        'z': z_np,
    ...
    plt.savefig('1D3_3d_dFdz.png')
    plt.close()

    savemat('1D3_3d_dFdz_data.mat', {
        'x': x_np,
        'z': z_np,
    ...
    for x_val in x_values:
        ...
        # Save F(z) at fixed x
        savemat(f'1D3_F_x{x_val}_data.mat', {
            'z': z_np[idx, :],
    ...
    plt.legend()
    plt.subplots_adjust(left=0.25, right=0.95, top=0.95, bottom=0.25)
    plt.savefig('1D3_plot_F_vs_z.png')
    plt.close()
    ...
    for x_val in x_values:
        ...
        savemat(f'1D3_dFdz_x{x_val}_data.mat', {
            'z': z_np[idx, :],
    ...
    plt.legend()
    plt.subplots_adjust(left=0.25, right=0.95, top=0.95, bottom=0.25)
    plt.savefig('1D3_plot_dFdz_vs_z.png')
    plt.close()
    ...
    plt.ylabel('Density')
    plt.tight_layout()
    plt.savefig('1D3_histogram_dFdz.png')
    plt.close()
    
    savemat('1D3_histogram_dFdz_data.mat', {
    ...
    plt.ylabel('U')
    plt.subplots_adjust(left=0.25, right=0.95, top=0.95, bottom=0.25)
    plt.savefig('1D3_U.png')  # Save the figure
    plt.close()

    savemat('1D3_U_data.mat', {
        'X': X,
        'U': U
    })
...
torch.save(trained_model.state_dict(), "model_parameters1D3.pth")

# To load the model parameters from a file
trained_model.load_state_dict(torch.load("model_parameters1D3.pth"))
