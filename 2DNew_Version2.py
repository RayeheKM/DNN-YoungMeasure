...
# --- Global font settings for publication quality (Times New Roman, LaTeX math) ---
plt.rcParams.update({
    "font.family": "DejaVu Serif",  # A built-in serif font that works everywhere
...
def visualize_model_updated_minibatch(model, loss_history, term1_history, term2_history, term3_history, x, y, z, w):
    ...
    plt.savefig('2DNew_loss.pdf', bbox_inches='tight', transparent=True)
    ...
    for i, (x_fixed, y_fixed) in enumerate(fixed_points):
        ...
        # Plot F
        fig = plt.figure(figsize=(5, 3.5), dpi=600, facecolor='white')  # White figure background
        ax = fig.add_subplot(111, projection='3d', facecolor='white')  # White axes background
        
        # Surface plot with single-color gradient
        ax.plot_surface(Z.detach().cpu().numpy(),
                        W.detach().cpu().numpy(),
                        F.detach().cpu().numpy(),
                        cmap='cividis')
        ...
        # Grid and limits
        ax.grid(True, color='gray', linestyle='--', linewidth=0.01, alpha=0.01)  # Very transparent grid
        
        # Layout and save
        plt.tight_layout()
        plt.savefig(f'2DNew_3d_F_x{x_fixed}_y{y_fixed}.pdf')
        plt.close(fig)

    

        # Plot dF/dz
        fig = plt.figure(figsize=(5, 3.5), dpi=600)  # White figure background
        ax = fig.add_subplot(111, projection='3d')  # White axes background
        ax.set_facecolor("white")           # 2D background of axes (sometimes works for 3D too)
        ax.tick_params(pad=0)
        fig.patch.set_facecolor("white")    # outer figure background
        
        # To really fix the 3D gray panes:
        ...
        ax.plot_surface(Z.detach().cpu().numpy(),
                        W.detach().cpu().numpy(),
                        dF_dz.detach().cpu().numpy(),
                        cmap='Greens')  # Use 'Greens' for a clean gradient
        
        # Axis labels
        ax.set_xlabel(r'$\xi$')
        ax.set_ylabel(r'$\tau$')
        ax.set_zlabel(r'$∂F/∂\xi$')  # Increased padding for visibility
        ...
        # Layout and save
        plt.tight_layout()
        plt.savefig(f'2DNew_3d_dFdz_x{x_fixed}_y{y_fixed}.pdf')
        plt.close(fig)
        ...
        ax.plot_surface(Z.detach().cpu().numpy(),
                        W.detach().cpu().numpy(),
                        dF_dw.detach().cpu().numpy(),
                        cmap='Oranges')  # Use 'Blues' for a clean gradient
        
        # Axis labels
        ax.set_xlabel(r'$\xi$')
        ax.set_ylabel(r'$\tau$')
        ax.set_zlabel(r'$∂F/∂\tau$')  # Increased padding for visibility
        ...
        ax.xaxis._axinfo["grid"]['color'] = (0.5, 0.5, 0.5, 0.1)  # RGBA tuple, with alpha=0.1
        ...
        # Layout and save
        plt.tight_layout()
        plt.savefig(f'2DNew_3d_dFdw_x{x_fixed}_y{y_fixed}.pdf')
        plt.close(fig)
    ...
    for col, (x_value, y_value) in enumerate(fixed_points):
        ...
        fig = plt.figure(figsize=(2.3, 1.6), dpi=600)
        plt.hist(dF_dz.detach().cpu().numpy(), bins=20, alpha=0.5, color='red', density=True)
        plt.xlim(-1.5, 1.5)
        plt.xlabel('dF/dz')
        plt.ylabel('Density')
        plt.tight_layout()
        plt.savefig(f'2DNew_histogram_dFdz_x{x_value}_y{y_value}.pdf', bbox_inches='tight', transparent=True)
        plt.close(fig)
        ...
        plt.ylabel('Density')
        plt.tight_layout()
        plt.savefig(f'2DNew_histogram_dFdw_x{x_value}_y{y_value}.pdf', bbox_inches='tight', transparent=True)
        plt.close(fig)
    ...
    # Plot U vs. x and y
    fig = plt.figure(figsize=(3, 2.6), dpi=600)
    ...
    ax.grid(True, color='gray', linestyle='--', linewidth=0.3, alpha=0.2)
    fig.subplots_adjust(left=0.01, right=0.8, bottom=0.1, top=1)
    plt.savefig('2DNew_U.pdf', bbox_inches='tight', transparent=True)
    plt.close()
...
torch.save(trained_model.state_dict(), "model_parametersSemi42.pth")

# To load the model parameters from a file
trained_model.load_state_dict(torch.load("model_parametersSemi42.pth"))